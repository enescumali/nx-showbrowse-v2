...
- If there's no live snapshot on disk (a genuinely first-ever boot, or a fresh container on a host with an ephemeral filesystem), it falls back to `apps/api/data/seed-snapshot.json.gz` — a real, full crawl result committed to the repo (`apps/api/src/ingestion/seed-snapshot.ts`) — and serves from that instantly instead of re-crawling. It then persists the seed to the live snapshot path so a restart within that same container's lifetime, and the daily incremental sync, build on it directly. Only if neither exists does it crawl TVMaze's index from page 0, rate-limited under TVMaze's documented ~20 calls/10s per IP, backing off and retrying on 429.
...
**Deploying it:** `render.yaml` at the repo root is a Render Blueprint — connect the repo on Render ("New +" → "Blueprint"), and it picks up the build command (`npm ci --include=dev`, since `nx`/`tsx` are dev dependencies this runtime needs even in production), start command (`npx nx run api:start`), health check path (`/health`), and env vars (`ADMIN_TOKEN` generated by Render, the rest defaulted) automatically. The committed seed snapshot means the very first deploy already serves real data instead of spending its first few minutes crawling. `apps/web`'s production `VITE_BFF_API_BASE_URL` (set in Vercel's project env vars, not the committed `.env`) then points at the resulting `https://<service>.onrender.com` URL.
