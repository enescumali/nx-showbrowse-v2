...
- Clicking a show opens a quick-view panel — a side panel on desktop, a bottom sheet on mobile — with a "View full page" link to the complete show detail page (cast, full overview); the underlying grid, its filters, and scroll position stay untouched behind it
...
### Why a quick-view panel instead of always navigating to the detail page

Clicking a show used to always navigate to `/shows/:id`, leaving whatever grid you were browsing (and its filters, sort, page, scroll position) entirely behind. `ShowQuickView.vue` (`apps/web/src/components/`) now opens as an overlay instead — a side panel on desktop, a bottom sheet on mobile, same component, direction switching at the `md` breakpoint via a CSS media query — while the page underneath stays mounted and untouched.

It's driven by a `?show=` query param on the _current_ route (`useQuickView.ts`), not a separate route: opening a show pushes `?show=5` onto wherever you already are, so `/catalog?genre=Drama&show=5` is itself a real, shareable, bookmarkable URL, and the browser's own back button closes the panel for free (it's just popstate changing the reactive route underneath). Closing explicitly (Escape, backdrop click, ✕) replaces the query instead, so there's no dangling history entry once you're done.

The full `/shows/:id` page still exists unchanged — the panel's "View full page" link goes there for the complete cast list and layout, and it's still what direct links, refreshes, and search-engine crawlers land on. Both share the same rendering logic via `ShowDetailContent.vue`, a small presentational component that takes a `Show | ShowDetail` and renders title/rating/genres/overview/cast — extracted so the full page and the panel can't drift out of sync with each other, rather than maintaining the same ~100 lines of markup twice.

As a real modal, it also does what the ARIA dialog pattern expects: focus moves into the panel on open and back to the triggering thumbnail on close, Tab is trapped within it, Escape closes it, and the dimmed background is made `inert` (not just visually dimmed) so it's genuinely unreachable by keyboard or a screen reader's virtual cursor while the panel is open.
...
- If there's no live snapshot on disk (a genuinely first-ever boot, or a fresh container on a host with an ephemeral filesystem), it falls back to `apps/api/data/seed-snapshot.json.gz` — a real, full crawl result committed to the repo (`apps/api/src/ingestion/seed-snapshot.ts`) — and serves from that instantly instead of re-crawling. It's deliberately not re-persisted to the live snapshot path at that point — re-stringifying the whole array right after decompressing and parsing it would double peak memory at boot, the worst possible moment on a memory-constrained host. The next daily incremental sync (or a manual `/admin/refresh`) writes the live snapshot the first time it runs. Only if neither a live snapshot nor the seed exists does it crawl TVMaze's index from page 0, rate-limited under TVMaze's documented ~20 calls/10s per IP, backing off and retrying on 429.
- A daily cron job (default `0 3 * * *`) then keeps it fresh incrementally: per TVMaze's own docs, it resumes from `floor(highestKnownShowId / 250)` instead of re-crawling everything, merging new/updated shows into the existing set. (On the current Render free-tier deployment specifically, this doesn't actually keep the catalog fresh in practice — see Known limitations.)
...
**Deploying it:** `render.yaml` at the repo root is a Render Blueprint — connect the repo on Render ("New +" → "Blueprint"), and it picks up the build command (`npm ci --include=dev`, since `tsx` is a dev dependency this runtime needs even in production), start command, health check path (`/health`), and env vars (`ADMIN_TOKEN` generated by Render, the rest defaulted) automatically. The start command runs `tsx` directly (`npx tsx --tsconfig apps/api/tsconfig.json apps/api/src/main.ts`) rather than through `nx run api:start` — the Nx CLI computes a full project graph and loads dev-only plugins (`@nx/vite`, `@nx/vitest`, `@nx/eslint`) on every boot, memory overhead a running production process has no use for, which matters on a free tier's 512MB cap. The committed seed snapshot means the very first deploy already serves real data instead of spending its first few minutes crawling. `apps/web`'s production `VITE_BFF_API_BASE_URL` (set in Vercel's project env vars, not the committed `.env`) then points at the resulting `https://<service>.onrender.com` URL.
...
Tests cover home navigation (genre carousels, a "See all" row linking into All Shows), search (URL params), the quick-view panel (opens on thumbnail click without navigating away, "View full page" reaches the real detail route, Escape closes it, and a real bounding-box check confirms it renders as a bottom sheet on a mobile viewport vs. a side panel on desktop), the detail page itself, All Shows' pagination/genre-filtering/sorting/deep-linking, and today's country filtering. `/shows/:id`, `/search`, and `/schedule/:country` still hit live TVMaze (they're proxied, not bulk-crawled), same as before.
